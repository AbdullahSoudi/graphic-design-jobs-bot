...
def format_job_message(job: Job) -> str:
    ...
    # For jsearch, show the original publisher (LinkedIn, Indeed, etc.)
    if job.source == "jsearch" and job.tags:
        source_display = job.tags[0]  # e.g. "LinkedIn", "Indeed"
    else:
        source_display = job.source.title()

    lines.append(f"\n<i>Source: {escape_html(source_display)}</i>")
